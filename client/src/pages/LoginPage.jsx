    ...
    <div className='min-h-screen bg-cover bg-center flex items-center justify-center gap-8 sm:justify-evenly max-sm:flex-col backdrop-blur-2xl'>
      ...
      <form onSubmit={onSubmitHandler} className='border-2 bg-white/8 text-white border-gray-500 p-6 flex flex-col gap-6 rounded-lg shadow-lg '>
        ...
        {currState === "Sign up" && !isDataSubmitted && ( 
          <input onChange={(e) => setFullName(e.target.value)} value={fullName} type="text" className='p-2 border border-gray-500 rounded-md focus:outline-none' placeholder='Enter your full name' required/>
        )}
        ...
        {
          currState === "Sign up" && isDataSubmitted && (
            <textarea rows={4} className='p-2 border border-gray-500 rounded-md focus:outline-none focus:ring-2 focus:ring-indigo-500' placeholder='Create a short bio...'required onChange={(e) => setBio(e.target.value)} value={bio}>
        ...
        <button type='submit' className='py-3 bg-gradient-to-r from-purple-400 to-violet-600 text-white rounded-md cursor-pointer'> 
          {currState === "Sign up" && isDataSubmitted ? "Create an Account" : currState ==='Sign up' && !isDataSubmitted ? 'Next' : "Login"}
        </button>

        { currState === 'Sign up' && isDataSubmitted && (<div className='flex items-center gap-2 text-sm text-gray-500'>
